(() => {
  ...
  function installStyles() {
    ...
    style.textContent = `
      ...
      #${C.overlayId} .rank-head{display:grid;grid-template-columns:46px 42px minmax(100px,1fr);gap:2px;align-items:center;margin-bottom:0;padding:0;color:#fde68a;font-size:11px;font-weight:1000;letter-spacing:0;text-align:center;text-transform:uppercase;text-shadow:1px 1px 0 #000;opacity:.92}
      ...
      #${C.overlayId} .rank-locker{display:grid;grid-template-columns:46px 42px minmax(100px,1fr);gap:2px;align-items:center;height:100%;min-height:19px;padding:1px;border:1px solid rgba(148,163,184,.2);border-radius:4px;background:rgba(15,23,42,.74);box-shadow:inset 0 1px 0 rgba(255,255,255,.08);font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,"Liberation Mono",monospace}
      ...
      @media(max-width:820px){
        #${C.overlayId}{--rank-board-w:210px}
  ...
  function fitKarts(operatorCount) {
    ...
    const rankBoardWidth = clamp(Math.round(window.innerWidth * 0.19), 218, 252);
  ...
  function rankBoardHtml(operators) {
    ...
    if (!rows.length) {
      return '<div class="rank-head"><span>LOCKER</span><span>POS</span><span title="Hours / Projected Hours">HOUR/PROJ</span></div><div class="rank-empty">Awaiting active operators</div>';
    }

    return `
      <div class="rank-columns">
        <div class="rank-column">
          <div class="rank-head"><span>LOCKER</span><span>POS</span><span title="Hours / Projected Hours">HOUR/PROJ</span></div>
          <div class="rank-list">
            ${rows.map(operator => {
